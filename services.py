...
def get_queries_by_qstring(db: sqlalchemy.orm.Session, qstring: str):
    return db.query(models.PrevQuery).filter(models.PrevQuery.qstring == qstring)

# gets all queries with the same link
def get_queries_by_link(db: sqlalchemy.orm.Session, link: str):
...
# def get_tag(db: sqlalchemy.orm.Session, link: str, qstring: str):
#     return db.query(models.PrevQuery).filter(models.PrevQuery.tag).first()

# def create_prev_query(db: sqlalchemy.orm.Session, prev_query: schemas.PrevQueryCreate):
